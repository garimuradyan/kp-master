...
window.KP_APP_VERSION='v52-direct-rest-login';
...
async function rpcSafe(fn,args){
  // V52: вход и проверка идут напрямую через REST RPC, без Supabase JS cache/обёртки
  try{
    var r=await fetch(SURL+'/rest/v1/rpc/'+fn,{
      method:'POST',
      headers:{
        apikey:SKEY,
    ...
    var data=null;
    try{data=text?JSON.parse(text):null;}catch(parseErr){data={ok:false,message:text||'Пустой ответ Supabase'};}
    if(!r.ok){
      console.error('REST RPC HTTP error',fn,r.status,data);
      return {data:{ok:false,message:(data&&data.message)||('Ошибка Supabase HTTP '+r.status)},error:data||{message:'HTTP '+r.status}};
    }
    return {data:data,error:null};
  }catch(e){
    console.error('REST RPC network exception',fn,e);
    return {data:{ok:false,message:'Нет соединения с Supabase'},error:e};
  }
}
...
async function doLogin(){
  try{
    ...
    var res=await rpcSafe('kp_login',{p_key:key,p_device_id:getDeviceId()});
    if(res.error||rpcFailed(res)){setAuthMsg(rpcMsg(res,(res.error&&res.error.message)||'Ключ не найден'));return;}
...
async function verifyKey(key,keyId,silent){
  ...
  var res;
  try{res=await rpcSafe('kp_verify',{p_key:key,p_key_id:keyId,p_device_id:getDeviceId()});}
