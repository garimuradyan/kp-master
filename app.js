...
window.KP_APP_VERSION='v50-login-fixed';
...
async function rpcSafe(fn,args){
  ...
  try{
    if(sb&&sb.rpc){
      first=await sb.rpc(fn,args);
      if(!first.error)return first;
      console.error('Supabase RPC error:',fn,first.error);
    }
  }catch(e){
    console.error('Supabase RPC exception:',fn,e);
  }
  try{
    ...
    if(!r.ok){
      console.error('REST RPC error:',fn,data);
      return {data:data||{ok:false,message:'Ошибка RPC'},error:data||{message:'Ошибка RPC'}};
    }
    return {data:data,error:null};
  }catch(e2){
    console.error('REST RPC exception:',fn,e2);
    return first||{data:{ok:false,message:'Ошибка соединения с базой'},error:e2};
  }
}
